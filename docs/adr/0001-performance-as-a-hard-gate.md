...
mobile data; reaching them is the project's civic purpose. We are promoting performance from a
soft Lighthouse warning (≥ 0.7) to a hard, build-failing gate alongside accessibility: Lighthouse
mobile ≥ 0.9, ≤ ~50 KB gzipped first-party JS per page, progressive enhancement (pages work without
JS), one subset web font, LCP ≤ 2.5 s on slow 4G. Interactive data pages (Leaflet map, Chart.js
statistics) are a documented exception — heavy libraries load on user intent, not first paint.
