...
The full-width red bar is in the homepage markup, not shared chrome. Demote it to neutral-dark (principle #6: no large non-SDG fills).

**Files:**
- Modify: the hotline-bar CSS rule (locate first) — likely `assets/css/style.css` or `assets/css/home-redesign.css`
- Possibly: `src/index.njk` (the bar's container, if an inline style/class needs adjusting)

- [ ] **Step 1: Locate the bar**

Run: `grep -rniE "0927 400 8033|Police|emergency|hotline" src/index.njk` to find the markup + its wrapper class. Then `grep -rniE "<wrapper-class>" assets/css/` to find the CSS rule and its current red `background`.
