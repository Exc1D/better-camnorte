/* BetterCamNorte - Main JavaScript */
...
function showInstallBanner() {
  ...
  banner.innerHTML =
    ...
    '<button class="pwa-install-btn" aria-label="Install BetterCamNorte app">Install</button>' +
...
// Colour each area of the site by a Sustainable Development Goal, echoing the
// homepage's per-section wheel. One root var (--sdg) drives the section stars,
// icons and accents. Homepage sets --sdg per section in CSS, so it is skipped.
(function () {
  var GOAL = {
    services: '#fd6925', // 9
    'service-details': '#fd6925',
    government: '#19486a', // 17
    legislative: '#00689d', // 16
    news: '#dd1367', // 10
    budget: '#a21942', // 8
    statistics: '#fd9d24', // 11
    contact: '#26bde2', // 6
    faq: '#c5192d', // 4
  };
  var area = location.pathname.split('/').filter(Boolean)[0] || '';
  if (GOAL[area]) document.documentElement.style.setProperty('--sdg', GOAL[area]);
})();

// ===========================================================================
// SDG festivity motion — shared layer for every page (this file is deferred
// and already loaded site-wide). Progressive enhancement only: if motion is
// disallowed we bail immediately and leave the page fully visible and static.
...
(function () {
  ...
  var SDG = ['#E5243B', '#DDA63A', '#4C9F38', '#C5192D', '#FF3A21', '#26BDE2',
    '#FCC30B', '#A21942', '#FD6925', '#DD1367', '#FD9D24', '#BF8B2E', '#3F7E44',
    '#0A97D9', '#56C02B', '#00689D', '#19486A'];
  var ci = (Math.random() * 17) | 0;
  function nextSDG() { return SDG[ci++ % 17]; }
  ...
  function maskHeading(h) {
    if (h.dataset.sdgMask || h.closest('.hero, .site-header, header[role="banner"]')) return;
    h.dataset.sdgMask = '1';
    var clip = document.createElement('span');
    clip.className = 'sdg-clip';
    var inner = document.createElement('span');
    inner.className = 'sdg-inner';
    while (h.firstChild) inner.appendChild(h.firstChild);
    clip.appendChild(inner);
    h.appendChild(clip);
    h.classList.add('sdg-mask');
    io.observe(h);
  }
  ...
  function countUp(el) {
    ...
    var target = attr && !isNaN(parseFloat(attr)) ? parseFloat(attr) : parseFloat(numStr);
  ...
  function run() {
    // 1. line-mask the headings (the Tres Mares signature)
    [].forEach.call(document.querySelectorAll(
      'h1, .page-header-title, .section-header h2, ' +
      '.home-section-header h2, h2.section-title, .section-title'), maskHeading);

    // 2. header blocks reveal as groups (+ ribbon seam via CSS ::before)
    [].forEach.call(document.querySelectorAll(
      '.page-header, .section-header, .home-section-header'), function (g) {
      if (g.closest('.hero')) return;
      g.classList.add('sdg-reveal-group');
      io.observe(g);
    });
    ...
    if (px.length) {
      lenis.on('scroll', function () {
        ...
        px.forEach(function (el) {
          ...
          var d = parseFloat(el.getAttribute('data-parallax')) || 0.06;
          var c = r.top + r.height / 2 - vh / 2;
          el.style.transform = 'translate3d(0,' + (c * -d).toFixed(1) + 'px,0)';
        });
      });
    }
  }
